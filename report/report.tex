...
display. O resultado do mapeamento é verificado na figura \ref{fig:lcd}.
...
\subsubsection{Resultados Teste Preliminar}
% histórico no git
% resultado testes preliminares
É necessário apresentar mais a fundo alguns detalhes da implementação para
então compreender os resultados obtidos.

A primeira observação feita é pela opção da estrutura do código, dividido entre
uma biblioteca onde foram modeladas classes para a representação do
\textit{derailleur} e da roda de bicicleta. Esta opção se pauta em dois
princípios, um técnico e outro filosófico: utilizar a característica de
representação do mundo físico oferecida pelo programação orientada a objetos; a
posterior redistribuição do código como software livre, possibilitando a
expansão e melhoria do projeto. Esta organização é representada por:
...
O arquivo $keywords.txt$ é o arquivo de configuração para o destaque das
palavras chaves introduzidas pela biblioteca na IDE do Arduino.
...
\begin{figure}[!h]
\begin{center}
  \includegraphics[width=5.5in]{ranhura}
\end{center}
...
\begin{figure}[ht!]
\begin{center}
 \includegraphics[width=5.5in]{derra}
\end{center}
